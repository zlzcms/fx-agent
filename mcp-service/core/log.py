...
def set_custom_logfile() -> None:
    ...
    # 日志文件
    log_access_file = os.path.join(settings.LOG_DIR, settings.LOG_ACCESS_FILENAME)
    log_error_file = os.path.join(settings.LOG_DIR, settings.LOG_ERROR_FILENAME)
    ...
    def compression(filepath):
        ...
        if "-" in original_filename:
            return os.path.join(settings.LOG_DIR, f"{original_filename}.log")
        return os.path.join(
            settings.LOG_DIR,
            f"{original_filename}_{timezone.now().strftime('%Y-%m-%d')}.log",
        )
