...
if [ -f "$INDEX_HTML_PATH" ]; then
    ...
    # 创建构建时间显示的HTML代码（单行格式）
    BUILD_TIME_HTML="<div id='build-time-info' style='position: fixed; bottom: 40px; left: 10px; width: 236px; text-align: center; background: rgba(0,0,0,0.7); color: white; padding: 8px 12px; border-radius: 4px; font-size: 10px; z-index: 9999; font-family: monospace;'>构建时间: $BUILD_TIME</div>"
    
    # 在</body>标签前插入构建时间信息
    sed -i.bak "s|</body>|$BUILD_TIME_HTML</body>|g" "$INDEX_HTML_PATH"
...
echo "正在创建服务器目标目录..."
if [ "$USE_PASSWORD" = true ]; then
    sshpass -p "$SSH_PASSWORD" ssh "$SERVER_USER@$SERVER_HOST" "mkdir -p $SERVER_PATH"
else
