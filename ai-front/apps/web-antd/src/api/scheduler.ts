...
import { CeleryTaskStatus } from '../types/celery';
...
export interface TaskResult {
  id: number;
  task_id: string;
  status: CeleryTaskStatus | string; // 支持Celery标准状态枚举
  result?: string;
  date_done?: string;
  traceback?: string;
  name?: string;
...
export async function getTaskResultListApi(params?: TaskResultParams) {
  return requestClient.get<PaginationResult<TaskResult>>('/api/v1/tasks/results', {
    params,
  });
}
...
export async function getAllTaskSchedulerApi() {
  return requestClient.get<TaskSchedulerResult[]>('/api/v1/tasks/schedulers/all');
}

export async function getTaskSchedulerApi(pk: number) {
  return requestClient.get<TaskSchedulerResult>(`/api/v1/tasks/schedulers/${pk}`);
}
...
export async function updateTaskSchedulerApi(pk: number, data: CreateTaskSchedulerParams) {
