...
import type { CodeGenColumnParams, CodeGenColumnResult } from '#/plugins/code_generator/api';
...
function onActionClick({ code, row }: OnActionClickParams<CodeGenColumnResult>) {
  switch (code) {
    case 'delete': {
  ...
  confirm({
    ...
    content: h('div', null, [
      h(Alert, {
        type: 'error',
        showIcon: false,
        message: '代码生成将进行磁盘文件（覆盖）写入，切勿在生产环境中使用！！！',
      }),
      h('br'),
      h(List, {
        bordered: true,
        dataSource: paths,
        header: '即将写入以下文件：',
        renderItem: ({ item }) => h(List.Item, null, h('p', { class: 'text-orange-500' }, item)),
      }),
    ]),
    icon: 'error',
    confirmText: '不怂！就是干',
    ...
    <template #extra>
      <a-alert :show-icon="false" class="mr-3">
        <template #message>
          ...
          默认时间列状态：
          <a-tag :color="drawerApi.getData().default_datetime_column ? 'success' : 'warning'">
