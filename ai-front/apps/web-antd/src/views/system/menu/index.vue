...
import { createSysMenuApi, deleteSysMenuApi, getSysMenuTreeApi, updateSysMenuApi } from '#/api';
...
const modalTitle = computed(() => {
  return formData.value?.id ? $t('@sys-menu.editMenu') : $t('@sys-menu.createMenu');
});
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <a-button class="mr-2" type="primary" @click="expandAll">
          {{ $t('@sys-menu.expandAll') }}
        </a-button>
        <a-button type="primary" @click="collapseAll">{{ $t('@sys-menu.collapseAll') }}</a-button>
      </template>
      <template #title_default="{ row }">
        <div class="flex w-full items-center gap-1">
          <div class="size-5 flex-shrink-0">
            <IconifyIcon v-if="row.type === 2" icon="carbon:security" class="size-full" />
            <a-image
              v-else-if="/^https?:\/\/.*$/.test(row.icon || '')"
              :src="row.icon"
              class="size-full"
            />
            <IconifyIcon v-else :icon="row.icon || 'carbon:circle-dash'" class="size-full" />
          </div>
