...
<template>
  <ColPage auto-content-height :resizable="false" :left-width="20" :right-width="80">
    <template #left>
        ...
        <div class="-mt-3 p-3">
          <AiAction
            ...
            :ai="{
              ...
              askContent: '请帮我分析近30天用户交易数据',
              id: 'user-transaction-data',
            }"
          ...
          >
            <template #title="{ name }">
              <span v-if="name.includes(searchDeptValue)">
                ...
                {{ name.substring(name.indexOf(searchDeptValue) + searchDeptValue?.length) }}
              </span>
    ...
    <Grid>
      ...
      <template #dept="{ row }">
        ...
        <span v-else>{{ $t('@sys-user.notBound') }}</span>
      </template>
      <template #roles="{ row }">
    ...
    <editModal :title="$t('@sys-user.editUser')">
      <EditForm />
    </editModal>
    <addModal :title="$t('@sys-user.addUser')">
      <AddForm />
    </addModal>
    <resetPwdModal :title="$t('@sys-user.resetPassword')">
      <ResetPwdForm />
    </resetPwdModal>
  </ColPage>
</template>
