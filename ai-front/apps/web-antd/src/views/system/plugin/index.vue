...
function downloadConfirm(plugin: string) {
  confirm({
    icon: 'success',
    content: $t('@sys-plugin.confirmPackageDownload'),
  }).then(async () => {
    try {
...
function deleteConfirm(plugin: string) {
  confirm({
    icon: 'warning',
    content: $t('@sys-plugin.confirmUninstall'),
  }).then(async () => {
    try {
...
<template>
  <Page>
    <VbenButton @click="() => modalApi.open()">
      {{ $t('@sys-plugin.installPlugin') }}
    </VbenButton>
    <Modal :title="$t('@sys-plugin.installPlugin')">
      <Form />
    </Modal>
    <a-alert
      v-show="pluginChanged"
      class="mt-4"
      :message="$t('@sys-plugin.pluginStatusChangeAlert')"
      type="warning"
      show-icon
    />
    <div class="mt-4">
        ...
        <a-card v-for="info in pluginInfo" :key="info.plugin.name">
          ...
          <template #extra>
            <a-switch
              ...
              checked-value="1"
              :checked-children="$t('@sys-plugin.enable')"
              :un-checked-children="$t('@sys-plugin.disable')"
