...
import { createSysDeptApi, deleteSysDeptApi, getSysDeptTreeApi, updateSysDeptApi } from '#/api';
...
const modalTitle = computed(() => {
  return formData.value?.id ? $t('@sys-dept.editDept') : $t('@sys-dept.createDept');
});
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <a-button class="mr-2" type="primary" @click="expandAll">
          {{ $t('@sys-dept.expandAll') }}
        </a-button>
        <a-button type="primary" @click="collapseAll">{{ $t('@sys-dept.collapseAll') }}</a-button>
      </template>
    </Grid>
