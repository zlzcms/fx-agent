...
watch(
  ...
  (newVal) => {
    ...
    if (newVal) {
      dataPermission.value = props.riskType as RiskType;
      loadUserData(dataPermission.value, '', 1, 50);
    }
  },
);
...
// 客户选择模式：'select' 为下拉选择，'manual' 为手动输入
const customerSelectMode = ref<'manual' | 'select'>('select');
const manualCustomerIds = ref<string>(''); // 手动输入的客户ID，用逗号分隔
...
const askContent = computed(() => {
  ...
  // 获取用户类型名称
  let userType = $t('@risk.risk-assistants.customer');
  switch (dataPermission.value) {
    case DATA_PERMISSION.AGENT_USER: {
      userType = $t('@risk.risk-assistants.agent');
      break;
    }
    case DATA_PERMISSION.CRM_USER: {
      userType = $t('@risk.risk-assistants.crmUser');
      break;
    }
    case DATA_PERMISSION.PAYMENT: {
      userType = $t('@risk.risk-assistants.paymentUser');
      break;
    }
    default: {
      userType = $t('@risk.risk-assistants.customer');
    }
  }
  ...
  const timeUnitMap: Record<TimeRangeType, string> = {
    day: $t('@risk.risk-assistants.days'),
    month: $t('@risk.risk-assistants.months'),
    quarter: $t('@risk.risk-assistants.quarters'),
    year: $t('@risk.risk-assistants.years'),
  };
  const timeDesc = `${$t('@risk.risk-assistants.recent')}${dataTimeValue.value}${timeUnitMap[dataTimeRangeType.value]}`;

  // 构建 askContent
  return `${$t('@risk.risk-assistants.pleaseUse')}${props.assistantName}${$t('@risk.risk-assistants.analyze')}${userType}${userNames}，${timeDesc}${$t('@risk.risk-assistants.riskData')}`;
});
...
function getTimeUnitLabel(): string {
  ...
  return unitLabels[dataTimeRangeType.value] || $t('@risk.risk-assistants.months');
}
        ...
        if (response && response.items) {
          sourceData.value = response.items.map((item: any) => ({
            ...
            title:
              ...
              item.nickname ||
              item.username ||
              `${$t('@risk.risk-assistants.agent')}${item.id}`,
          ...
          sourceData.value = response.items.map(
            (item: any): DataSourceItem => ({
              ...
              title:
                ...
                item.username ||
                item.email ||
                `${$t('@risk.risk-assistants.customer')}${item.id}`,
              description: item.email || '',
            }),
          );
          total.value = response.total || 0;
        ...
        if (response && response.items) {
          sourceData.value = response.items.map((item: any) => ({
            ...
            title:
              ...
              item.username ||
              item.email ||
              `${$t('@risk.risk-assistants.crmUser')}${item.id}`,
            description: item.email || '',
          }));
          total.value = response.total || 0;
        } else {
...
// 切换客户选择模式
const toggleCustomerSelectMode = () => {
  customerSelectMode.value = customerSelectMode.value === 'select' ? 'manual' : 'select';
  // 切换模式时清空之前的选择
  if (customerSelectMode.value === 'manual') {
    targetKeys.value = [];
    manualCustomerIds.value = '';
  } else {
    manualCustomerIds.value = '';
    targetKeys.value = [];
  }
...
function closeMockTraining(): void {
  ...
  targetKeys.value = [];
  manualCustomerIds.value = '';
  customerSelectMode.value = 'select';
}

// 验证并显示提示
function validateBeforeTest(): boolean {
  if (targetKeys.value.length === 0) {
    message.warning($t('@risk.risk-assistants.pleaseSelectUserFirst'));
    return false;
  }
  if (!dataTimeValue.value || dataTimeValue.value < 1) {
    message.warning($t('@risk.risk-assistants.pleaseEnterValidTimeValue'));
    return false;
  }
  return true;
}
...
<template>
  <Modal
    ...
    :title="$t('@risk.risk-assistants.mockTrainingTitle')"
    width="900px"
    :footer="null"
  ...
  >
      ...
      <div class="mb-6">
        <div>
          ...
          <ARow class="mb-4">
            <ACol :span="4">
              <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ $t('@risk.risk-assistants.dataPermissionRange') }}
              ...
              <div class="flex flex-col items-center">
                ...
                >
                  <option :value="RiskType.ALL_EMPLOYEE">
                    {{ $t('@risk.risk-assistants.customer') }}
                  </option>
                  <option :value="RiskType.AGENT_USER">
                    {{ $t('@risk.risk-assistants.agent') }}
                  </option>
                  <option :value="RiskType.CRM_USER">
                    {{ $t('@risk.risk-assistants.crmUser') }}
                  </option>
                  <option :value="RiskType.PAYMENT">
                    {{ $t('@risk.risk-assistants.paymentUser') }}
                  </option>
                </select>
              </div>
            </ACol>
            ...
            <ACol :span="4">
              <label class="block text-sm font-medium text-gray-700 mb-1">
                {{
                  ...
                  dataPermission === DATA_PERMISSION.PAYMENT
                    ? $t('@risk.risk-assistants.customer')
                    : dataPermission === DATA_PERMISSION.CRM_USER
                      ? $t('@risk.risk-assistants.crmUser')
                      : $t('@risk.risk-assistants.agent')
                }}
                <span class="text-red-500">*</span>
            ...
            <ACol :span="20">
              <div class="flex flex-col space-y-2">
                ...
                <div class="flex items-center space-x-2">
                  <button
                    type="button"
                    @click="toggleCustomerSelectMode"
                    ...
                    "
                  >
                    {{ $t('@risk.risk-assistants.dropdownSelect') }}
                  </button>
                  <button
                    type="button"
                    @click="toggleCustomerSelectMode"
                    class="px-3 py-1 text-xs font-medium rounded-md transition-colors"
                    :class="
                      customerSelectMode === 'manual'
                        ? 'bg-blue-100 text-blue-700'
                        : 'bg-gray-100 text-gray-600'
                    "
                  >
                    {{ $t('@risk.risk-assistants.manualInputId') }}
                  </button>
                </div>

                <!-- 下拉选择模式 -->
                <a-select
                  v-if="customerSelectMode === 'select'"
                  v-model:value="targetKeys"
                  mode="multiple"
                  :placeholder="
                    dataPermission === DATA_PERMISSION.ALL_EMPLOYEE ||
                    dataPermission === DATA_PERMISSION.PAYMENT
                      ? $t('@risk.risk-assistants.pleaseSelectCustomerOrSearch')
                      : dataPermission === DATA_PERMISSION.CRM_USER
                        ? $t('@risk.risk-assistants.pleaseSelectCrmUserOrSearch')
                        : $t('@risk.risk-assistants.pleaseSelectAgentOrSearch')
                  "
                  :loading="loading"
                  :show-search="true"
                  :filter-option="false"
                  :not-found-content="
                    loading
                      ? $t('@risk.risk-assistants.searching')
                      : searchQuery
                        ? $t('@risk.risk-assistants.noSearchResult')
                        : $t('@risk.risk-assistants.pleaseInputKeywordToSearch')
                  "
                  style="width: 100%"
                  @search="handleSearch"
                ...
                <!-- 手动输入模式 -->
                <div v-if="customerSelectMode === 'manual'" class="space-y-2">
                  <input
                    type="text"
                    v-model="manualCustomerIds"
                    :placeholder="`${$t('@risk.risk-assistants.pleaseInput')}${dataPermission === DATA_PERMISSION.ALL_EMPLOYEE || dataPermission === DATA_PERMISSION.PAYMENT ? $t('@risk.risk-assistants.customer') : dataPermission === DATA_PERMISSION.CRM_USER ? $t('@risk.risk-assistants.crmUser') : $t('@risk.risk-assistants.agent')}ID，${$t('@risk.risk-assistants.multipleIdsSeparatedByComma')}`"
                    class="w-full rounded-md border border-gray-300 py-2 px-3 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    @input="handleManualIdsChange(($event.target as HTMLInputElement)?.value || '')"
                  />
                  <p class="text-xs text-gray-500">{{ $t('@risk.risk-assistants.exampleIds') }}</p>
                </div>
              </div>
            </ACol>
          </ARow>
          ...
          <ARow class="mb-4">
            <ACol :span="4">
              <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ $t('@risk.risk-assistants.dataTimeRange') }} <span class="text-red-500">*</span>
              </label>
            </ACol>
            <ACol :span="20">
              <div class="flex items-center">
                <div style="width: 360px">
                  <select
                    ...
                    class="w-full rounded-md border border-gray-300 py-2 px-3 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  >
                    <option value="day">{{ $t('@risk.risk-assistants.byDay') }}</option>
                    <option value="month">{{ $t('@risk.risk-assistants.byMonth') }}</option>
                    <option value="quarter">{{ $t('@risk.risk-assistants.byQuarter') }}</option>
                    <option value="year">{{ $t('@risk.risk-assistants.byYear') }}</option>
                  </select>
                </div>
        ...
        <div class="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div class="mb-2">
            <span class="font-medium text-gray-700"
              >{{ $t('@risk.risk-assistants.testTip') }}:</span
            >
          </div>
          <div class="text-sm text-gray-600 mb-2">
            <span class="text-red-500">*</span> {{ $t('@risk.risk-assistants.requiredFieldsTip') }}
          </div>
          <div v-if="canStartTest" class="text-sm text-green-600">
            ...
            {{ $t('@risk.risk-assistants.currentTestQuestion') }}: {{ askContent }}
          </div>
          <div v-else class="text-sm text-gray-500">
            {{ $t('@risk.risk-assistants.completeConfigTip') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mt-6 flex justify-end space-x-3 px-6 pb-4">
      <AiAction v-if="canStartTest" :ai="aiConfig" base-url="https://client.ai1center.com">
        ...
        >
          {{ $t('@risk.risk-assistants.startMockTest') }}
        </button>
      </AiAction>
      <button
        v-else
