...
import { updateSysUserEmailApi, updateSysUserPasswordApi, updateSysUserPhoneApi } from '#/api';
...
<template>
  <a-list :data-source="securityOptions" :split="false" class="-ml-4">
    <template #renderItem="{ item }">
      <a-list-item>
        <template #actions>
          <VbenButton :variant="item.status ? 'outline' : 'default'" @click="openModal(item.type)">
            {{ item.status ? '修改' : '绑定' }}
          </VbenButton>
        </template>
        <a-list-item-meta :description="item.description">
          ...
          <template #title>
            {{ item.title }}
            <span class="ml-2 text-xs" :class="item.status ? 'text-green-500' : 'text-orange-500'">
              {{ item.statusString }}
            </span>
          </template>
        </a-list-item-meta>
