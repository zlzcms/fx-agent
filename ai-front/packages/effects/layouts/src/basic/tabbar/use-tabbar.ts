...
export function useTabbar() {
  ...
  function wrapperTabLocale(tab: RouteLocationNormalizedGeneric) {
    // 国际化处理：优先使用 sidebar.{routeName} 进行翻译
    const routeName = tab.name as string;
    const originalTitle = tab?.meta?.title as string;
    ...
    if (routeName && typeof routeName === 'string') {
      ...
      // 如果翻译后的值与key不同，说明找到了翻译
      if (translated !== i18nKey) {
        translatedTitle = translated;
      } else if (originalTitle) {
        // 如果没有找到sidebar翻译，尝试直接翻译title（兼容旧的翻译方式）
        translatedTitle = $t(originalTitle);
      }
    }
