...
const wrapperClass = 'ai-action-wrapper';
const iframeSrc = computed(() => {
  const ai = props.ai;
  const val = `${props.baseUrl}?token=${encodeURIComponent(ai?.token || '')}&askContent=${encodeURIComponent(ai?.askContent || '')}&locale=${encodeURIComponent(ai.locale || 'zh')}&channel=${encodeURIComponent(ai.id || '')}`;
  return val;
});
...
<template>
  ...
  <span
    ref="wrapperRef"
    :class="[wrapperClass, { 'ai-action-wrapper--clickable': isHovering }]"
    @mouseenter="onEnter"
...
/* 按钮模式：添加可点击样式 */
.ai-action-wrapper--clickable {
  cursor: pointer;
  transition: opacity 0.2s ease;
}

.ai-action-wrapper--clickable:hover {
  opacity: 0.8;
}

/* 默认触发器按钮样式 */
.ai-action__default-trigger {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  padding: 6px;
...
.ai-action__default-trigger:hover {
  color: #4f46e5;
  background: #e0e7ff;
  border-color: #a5b4fc;
...
.ai-action__default-trigger:active {
