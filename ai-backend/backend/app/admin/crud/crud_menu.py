...
class CRUDMenu(CRUDPlus[Menu]):
    ...
    async def get(self, db: AsyncSession, menu_id: int) -> Menu | None:
        ...
        :return:
        """
        return await self.select_model_by_column(db, id=menu_id, deleted_at=None)
    ...
    async def get_by_title(self, db: AsyncSession, title: str) -> Menu | None:
        ...
        :return:
        """
        return await self.select_model_by_column(db, title=title, type__ne=2, deleted_at=None)
    ...
    async def get_sidebar(self, db: AsyncSession, menu_ids: list[int] | None) -> Sequence[Menu]:
        ...
        :return:
        """
        filters = {"type__in": [0, 1, 3, 4], "deleted_at": None}
    ...
    async def delete(self, db: AsyncSession, menu_id: int) -> int:
        ...
        :return:
        """
        return await self.update_model(db, menu_id, {"deleted_at": datetime.now()})
    ...
    async def get_children(self, db: AsyncSession, menu_id: int) -> list[Menu | None]:
        ...
        :return:
        """
        return await self.select_models(db, parent_id=menu_id, deleted_at=None)
