...
class UserService:
    ...
    @staticmethod
    async def get_select(*, dept: int | None, username: str | None, phone: str | None, status: int | None) -> Select:
        """
        获取用户列表查询条件
    ...
    @staticmethod
    async def get_all(*, status: int | None = None) -> Sequence[User]:
        """
        获取所有用户（不分页）
    ...
    async def create(*, request: Request, obj: AddUserParam) -> None:
        ...
        async with async_db_session.begin() as db:
            ...
            # 检查邮箱唯一性
            if obj.email and await user_dao.get_by_email(db, obj.email):
                raise errors.ConflictError(msg="邮箱已被使用")

            # 检查CRM用户ID唯一性
            if obj.crm_user_id and await user_dao.check_crm_user_id(db, obj.crm_user_id):
                raise errors.ConflictError(msg="CRM用户ID已存在")
    ...
    async def update(*, request: Request, pk: int, obj: UpdateUserParam) -> int:
        ...
        async with async_db_session.begin() as db:
            ...
            # 检查CRM用户ID唯一性
            if obj.crm_user_id and obj.crm_user_id != user.crm_user_id:
                if await user_dao.check_crm_user_id(db, obj.crm_user_id):
                    raise errors.ConflictError(msg="CRM用户ID已存在")
