...
class AuthService:
    ...
    async def user_verify(db: AsyncSession, username: str, password: str) -> User:
        ...
        if not user.is_staff:
            raise errors.AuthorizationError(msg="用户无权限登录后台管理")
    ...
    async def get_codes(*, request: Request) -> list[str]:
        ...
        if request.user.is_superuser:
            ...
            # 重新查询用户数据以确保关联属性可以正常访问
            async with async_db_session() as db:
                from backend.app.admin.crud.crud_user import user_dao

                user_with_relations = await user_dao.get_with_relation(db, user_id=request.user.id)
                if user_with_relations:
                    roles = user_with_relations.roles
                    if roles:
                        for role in roles:
