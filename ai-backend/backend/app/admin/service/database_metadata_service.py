...
# @Last Modified time: 2025-07-05 14:32:52
# !/usr/bin/env python3
# -*- coding: utf-8 -*-
import re
...
class DatabaseMetadataService:
                    ...
                    columns.append(
                        {
                            ...
                            "column_description": field_descriptions.get(column_name, "")
                            if field_descriptions.get(column_name, "") != "None"
                            else "",
                            "children_columns": [],
                        }
                    )
    ...
    async def process_linked_data_query(
        ...
        *,
        primary_result: dict,
        no_primary_sources: List[dict],
        data_time_range_type: Optional[str] = None,
        data_time_value: Optional[int] = None,
    ) -> dict:
        """
        处理链接数据查询
        ...
        try:
            ...
            for col in primary_columns:
                compress_data["columns"].append(
                    col.get("column_description") if col.get("column_description") else f"{col.get('column_name', '')}"
                )
            ...
            for row_item in primary_rows:
                ...
                for k, v in row_item.items():
                    row_value.append(v if v else "")
                # 获取链接字段的值
                # print('row_item=========',row_item)
                data_source_link_field_value = row_item.get(data_source_link_field)
                # print('data_source_link_field_value=========',data_source_link_field_value)
                ...
                for source_item in no_primary_sources:
                    ...
                    collection_column_name = f"相关联{collection_name}数据"
                    if collection_column_name not in compress_data["columns"]:
                        compress_data["columns"].append(collection_column_name)
                    ...
                    # 确保tables格式正确
                    sql_request = SqlGenerateRequest(
                        database_name=database_name,
                        tables=tables,
                        condition=condition,
                        data_time_range_type=data_time_range_type,
                        data_time_value=data_time_value,
                    )
                        ...
                        if len(query_result["rows"]) > 0:
                            ...
                            for col in query_result["columns"]:
                                if col.get("column_description", ""):
                                    ...
                                    children_data += f'"{col.get("column_name", "")}",'
                            # 去掉最后一个逗号
                    ...
                    if children_data != "":
                        row_value.append(f"这一行相关{collection_name}的数据如下:{children_data}")
                    else:
                        row_value.append("")
