...
if TYPE_CHECKING:
    from backend.app.admin.model import ApiKey, Dept, Role
...
class User(Base):
    ...
    crm_user_id: Mapped[str | None] = mapped_column(
        String(50), default=None, unique=True, index=True, comment="CRM系统用户ID"
    )
    ...
    # API Keys 一对多
    api_keys: Mapped[list[ApiKey]] = relationship(init=False, back_populates="user")

    # report_reads relationship removed as it's not used in the codebase
