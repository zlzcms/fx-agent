...
async def get_current_user(request: Request, db: CurrentSession) -> ResponseSchemaModel[GetUserInfoWithRelationDetail]:
    """获取当前用户信息，使用与用户列表接口相同的查询方式"""
    user = await user_dao.get_with_relation(db, user_id=request.user.id)
    if not user:
...
@router.get("/all", summary="获取所有用户列表(不分页)", dependencies=[DependsJwtAuth])
async def get_all_users(
    status: Annotated[int | None, Query(description="状态")] = None,
) -> ResponseSchemaModel[list[GetUserInfoWithRelationDetail]]:
    users = await user_service.get_all(status=status)
...
async def get_users_paged(
    ...
    page_data = await paging_data(db, user_select, schema=GetUserInfoWithRelationDetail)
