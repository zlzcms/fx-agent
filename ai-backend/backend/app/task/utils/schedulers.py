...
# 计划锁时长，避免重复创建 - 增加到30分钟以避免频繁的锁过期
DEFAULT_MAX_LOCK_TIMEOUT = 30 * 60  # 30 minutes in seconds
    ...
    def tick(self, **kwargs):
        ...
        if self.lock:
            ...
            try:
                ...
                logger.warning(f"Failed to extend lock: {e}. Attempting to re-acquire lock.")
                try:
                    ...
                    run_await(new_lock.acquire)()
                    self.lock = new_lock
                    logger.info("beat: Successfully re-acquired lock")
                except Exception as reacquire_error:
                    logger.error(f"Failed to re-acquire lock: {reacquire_error}. Continuing without lock.")
                    self.lock = None
...
def acquire_distributed_beat_lock(sender=None, **kwargs):
    ...
    try:
        ...
        logger.error(f"Failed to acquire beat lock: {e}")
        # Continue without lock - this allows beat to run but without distributed locking
        scheduler.lock = None
