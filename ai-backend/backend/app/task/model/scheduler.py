...
if TYPE_CHECKING:
    from backend.app.task.model.execution import TaskSchedulerExecution
...
class TaskScheduler(Base):
    ...
    user_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("sys_user.id", ondelete="SET NULL"), default=None, comment="用户ID"
    )
    subscription_id: Mapped[int | None] = mapped_column(
        ForeignKey("ai_subscription.id", ondelete="SET NULL"), default=None, comment="订阅ID"
    )
    priority: Mapped[int] = mapped_column(default=5, comment="任务优先级（1-10，数字越小优先级越高）")
    ...
    # 关联关系
    executions: Mapped[list[TaskSchedulerExecution]] = relationship(
        "TaskSchedulerExecution", back_populates="scheduler", cascade="all, delete-orphan", init=False
    )
