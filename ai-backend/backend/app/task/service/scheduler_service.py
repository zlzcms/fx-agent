...
class TaskSchedulerService:
    ...
    @staticmethod
    async def execute(*, pk: int, user_id: int | None = None, subscription_id: int | None = None) -> str:
        """
        执行任务
        ...
        async with async_db_session.begin() as db:
            ...
            try:
                ...
                execution_record = await task_scheduler_execution_dao.create(db, execution_data)

                # 为任务添加user_id上下文信息，用于权限验证
                if user_id:
                    kwargs["task_creator_id"] = user_id

                result = celery_app.send_task(name=task_scheduler.task, args=args, kwargs=kwargs)

                await task_scheduler_execution_dao.update(
                    db=db, pk=execution_record.id, obj_in={"celery_task_id": result.id}
                )
