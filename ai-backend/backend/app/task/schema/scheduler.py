...
class TaskSchedulerSchemeBase(SchemaBase):
    ...
    crontab: str | None = Field(default=None, description="运行的 Crontab 表达式")
    one_off: bool = Field(default=False, description="是否仅运行一次")
    priority: int = Field(default=5, description="任务优先级（1-10，数字越小优先级越高）")
...
class GetTaskSchedulerDetail(TaskSchedulerSchemeBase):
    ...
    @field_serializer(
        "last_run_time", "created_time", "updated_time", "start_time", "expire_time", when_used="unless-none"
    )
    def serialize_datetime_fields(self, value: datetime | None, _info) -> str | None:
        """将时间字段格式化为上海时区的字符串"""
        if value is None:
            return None
        try:
            ...
            # 确保转换为上海时区后再格式化
            shanghai_time = timezone.from_datetime(value)
            return timezone.to_str(shanghai_time)
