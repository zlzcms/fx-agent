...
def generate_time_range(
    time_range_type: str, time_value: str | int, fixed_end_time: Optional[datetime] = None
) -> Dict[str, str]:
    """
    ...
    Args:
        ...
        fixed_end_time: 固定的结束时间，避免重复计算时时间变化
    ...
    # 使用固定的结束时间，避免重复计算时时间变化
    end_time = fixed_end_time or datetime.now()
    ...
    try:
        ...
        if time_range_type == "day":
            ...
            # 处理日期溢出（如闰年的2月29日）
            try:
                start_time = datetime(end_time.year - time_value, end_time.month, end_time.day)
            except ValueError:
...
def get_mysql_date_range(start_date: Optional[str], end_date: Optional[str]) -> tuple:
    ...
    """
    mysql_start_date = convert_relative_date_to_mysql_format(start_date) if start_date else None
    mysql_end_date = convert_relative_date_to_mysql_format(end_date) if end_date else None
...
def get_start_and_end_time(parameters: Dict[str, Any], isstrptime=False):
    ...
    if parameters.get("start_time"):
        ...
        start_time = convert_relative_date_to_mysql_format(start_time) if start_time else None
        if isstrptime and start_time:
            start_time = int(time.mktime(time.strptime(start_time, "%Y-%m-%d %H:%M:%S")))
    if parameters.get("end_time"):
        end_time = parameters.get("end_time")
        end_time = convert_relative_date_to_mysql_format(end_time) if end_time else None
        if isstrptime and end_time:
...
def extract_date(self, text: str, date_type: str) -> Optional[str]:
    ...
    # 匹配相对时间表达式
    relative_pattern = (
        rf"(?:{date_type})(?:日期|时间)[是为:：]?\s*([^,，\s]+(?:天|周|月|年)前|当前日期|今天|昨天|上周|上个月|去年)"
    )
