...
def default_formatter(record):
    ...
    # 确保 correlation_id 字段存在
    if "correlation_id" not in record:
        record["correlation_id"] = settings.TRACE_ID_LOG_DEFAULT_VALUE
...
def setup_logging() -> None:
    ...
    """
    # 检测是否在 Celery worker 环境中运行
    # 如果是 Celery worker，则不进行日志配置，让 Celery 自己处理日志
    is_celery_worker = (
        os.environ.get("CELERY_WORKER_RUNNING") == "1"
        or "celery" in sys.argv[0]
        or os.path.basename(sys.argv[0]) == "celery"
        or any(arg.startswith("celery") for arg in sys.argv)
        or os.environ.get("SUPERVISOR_ENABLED") == "true"
        and os.environ.get("SUPERVISOR_PROCESS_NAME", "").startswith("celery")
    )

    if is_celery_worker:
        # 对于 Celery worker，只设置基本的日志重定向，不进行完整配置
        logging.basicConfig(level=settings.LOG_STD_LEVEL)
        return
    ...
    def correlation_id_filter(record):
        try:
            # 确保 correlation_id 字段总是存在
            if "correlation_id" not in record:
                record["correlation_id"] = settings.TRACE_ID_LOG_DEFAULT_VALUE

            # 尝试获取 correlation_id
            cid = correlation_id.get(settings.TRACE_ID_LOG_DEFAULT_VALUE)
            if cid and cid != settings.TRACE_ID_LOG_DEFAULT_VALUE:
                record["correlation_id"] = cid[: settings.TRACE_ID_LOG_LENGTH]
        except Exception:
            # 如果correlation_id不可用，使用默认值
            record["correlation_id"] = settings.TRACE_ID_LOG_DEFAULT_VALUE
        return record

    # 配置 loguru 处理器
    # 如果设置为CRITICAL或更高，则不输出任何日志到控制台
    if settings.LOG_STD_LEVEL in ["CRITICAL", "OFF"]:
        logger.configure(handlers=[])  # 不添加任何处理器
    else:
        logger.configure(
            handlers=[
                {
