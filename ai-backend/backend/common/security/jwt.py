...
async def jwt_auth(request: Request) -> HTTPAuthorizationCredentials | None:
    """
    JWT认证依赖，当DISABLE_AUTH为True时跳过认证
    """
...
# JWT认证依赖注入
DependsJwtAuth = Depends(jwt_auth)
...
def base64_encode_token(token: str) -> str:
    """
    Base64编码JWT Token
...
def base64_decode_token(encoded_token: str) -> str:
    """
    Base64解码JWT Token

    :param encoded_token: Base64编码的token
    :return: 解码后的JWT token
    """
    try:
        return base64.b64decode(encoded_token.encode("utf-8")).decode("utf-8")
    except Exception as e:
        raise errors.TokenError(msg=f"Token Base64解码失败: {str(e)}")
...
async def create_access_token(user_id: int, multi_login: bool, **kwargs) -> AccessToken:
    ...
    # Base64编码JWT Token
    encoded_access_token = base64_encode_token(access_token)

    if not multi_login:
        await redis_client.delete_prefix(f"{settings.TOKEN_REDIS_PREFIX}:{user_id}")

    # Redis中存储原始JWT token用于验证
    ...
    # 返回Base64编码后的token
    return AccessToken(access_token=encoded_access_token, access_token_expire_time=expire, session_uuid=session_uuid)
...
async def jwt_authentication(token: str) -> User:
    ...
    :param token: Base64编码的JWT token
    :return:
    """
