...
async def paging_data(db: AsyncSession, select: Select, schema: type[BaseModel] | None = None) -> dict[str, Any]:
    """
    基于 SQLAlchemy 创建分页数据

    :param db: 数据库会话
    :param select: SQL 查询语句
    :param schema: 可选的 Pydantic schema 类型，如果提供，会在会话活跃时将 SQLAlchemy 模型转换为该 schema
    :return:
    """
    paginated_data: _CustomPage = await apaginate(db, select)

    # 如果提供了 schema，在会话仍然活跃时转换 SQLAlchemy 模型为 Pydantic schema
    if schema is not None:
        converted_items = [schema.model_validate(item) for item in paginated_data.items]
        # 替换 items 为转换后的 Pydantic 模型
        paginated_data.items = converted_items
