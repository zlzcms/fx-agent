...
### ⚠️ IMPORTANT: Git Hooks Setup

**🚨 All developers MUST install Git hooks before starting development!**

Run this command immediately after cloning the repository:

```bash
./scripts/development/install-git-hooks.sh
```

This installs essential code quality checks:
- **Code formatting** (pre-commit)
- **Commit message format** (commit-msg)
- **Database migration checks** (pre-commit-alembic)

**Alternative development startup:**
```bash
# Use this script to start development - it will check hooks automatically
./scripts/development/dev-start.sh
```

### Git Commit

When committing code, the pre-commit hook will run Alembic migration checks. To ensure these checks work properly, you need to set up your PYTHONPATH correctly:
...
If you encounter "No module named 'backend'" errors during git commit, make sure your PYTHONPATH is set correctly.
